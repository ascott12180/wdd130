...
// button.addEventListener.Listener('click', search);
...
// Search function
function search() {
  const hikeQuery = searchInput.value.trim().toLowerCase();
  ...
  if (filteredHikes.length === 0) {
    hikeContainer.innerHTML = "<p>No hikes found.</p>";
    return;
  }
