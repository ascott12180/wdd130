...
let fitlerFruit = lowerSort.filter(searchFruit);
...
HTMLFormControlsCollection.log(productSort);
...
function searchList(item){
    return item.name.toLowerCase().inculdes(query.toLowerCase())
}
...
function searchTraits(item){
    return item.traits.find((trait) => trait.toLowerCase().inculdes(query.toLowerCase()))
}
